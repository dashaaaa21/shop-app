...
import { fetchByCollection, type Product } from '../api/products/products.api';
...
type CollectionSlug = 'wedding-collection' | 'party-collection' | 'halloween-collection';

const collectionMeta: Record<CollectionSlug, { title: string; subtitle: string; description: string; heroImage: string; accent: string }> = {
  'wedding-collection': {
    title: 'Wedding Collection', subtitle: 'AW 2026',
  ...
  'party-collection': {
    title: 'Party Collection', subtitle: 'AW 2026',
  ...
  'halloween-collection': {
    title: 'Halloween Collection', subtitle: 'AW 2026',
...
const CollectionPage = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchByCollection(slug).then((res) => setAllProducts(res.products)).finally(() => setLoading(false));
  }, [slug]);

  if (!slug || !meta) return <Navigate to="/" replace />;
        ...
        <div className="container">
          ...
          <section className="col-hero">
            <div className="col-hero__image">
              ...
              <div className="col-hero__overlay">
                <p className="col-hero__eyebrow" style={{ color: meta.accent }}>{meta.subtitle}</p>
          ...
          <section className="col-controls">
            <div className="col-controls__left">
              <p className="col-controls__count">{filtered.length} {filtered.length === 1 ? 'item' : 'items'}</p>
            </div>
            <div className="col-controls__right">
              <div className="col-category-tabs">
                {categories.map((cat) => (
                  <button key={cat} className={`col-category-tab ${categoryFilter === cat ? 'col-category-tab--active' : ''}`} onClick={() => setCategoryFilter(cat)}>
                    {cat === 'all' ? 'All' : cat}
                  </button>
                ))}
              </div>
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="col-sort-select" aria-label="Sort products">
          ...
          <section className="col-products">
            {loading ? <div className="col-loading" /> : filtered.length > 0 ? (
              <ProductGrid products={filtered} columns={3} />
            ) : (
              <div className="col-empty">
                <p>No items match the selected filter.</p>
                <button className="col-reset-btn" onClick={() => setCategoryFilter('all')}>Show All</button>
              </div>
            )}
          </section>
