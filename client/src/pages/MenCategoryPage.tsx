...
const categoryInfo = {
  suits: { title: 'Suits', description: 'Precision tailoring for the modern man', heroImage: 'https://images.unsplash.com/photo-1507679799987-c73779587ccf?w=1200&h=600&fit=crop&q=80', filter: 'Suits' },
  trousers: { title: 'Trousers', description: 'Refined trousers crafted for comfort and sophistication', heroImage: 'https://images.unsplash.com/photo-1624378439575-d8705ad7ae80?w=1200&h=600&fit=crop&q=80', filter: 'TROUSERS' },
  sweaters: { title: 'Sweaters', description: 'Premium knitwear and cozy essentials for every season', heroImage: 'https://images.unsplash.com/photo-1576566588028-4147f3842f27?w=1200&h=600&fit=crop&q=80', filter: 'SWEATERS' },
  outerwear: { title: 'Outerwear', description: 'Coats and jackets built to last', heroImage: 'https://images.unsplash.com/photo-1539533018447-63fcce2678e3?w=1200&h=600&fit=crop&q=80', filter: 'Outerwear' },
  shirts: { title: 'Shirts', description: 'From crisp dress shirts to relaxed linens', heroImage: 'https://images.unsplash.com/photo-1602810318383-e386cc2a3ccf?w=1200&h=600&fit=crop&q=80', filter: 'Shirts' },
  jackets: { title: 'Jackets', description: 'Versatile outerwear that moves with you', heroImage: 'https://images.unsplash.com/photo-1544022613-e87ca75a784a?w=1200&h=600&fit=crop&q=80', filter: 'Jackets' },
};
...
const MenCategoryPage = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchByCategory(currentCategory.filter, 'men')
      .then((res) => setAllProducts(res.products))
      .finally(() => setLoading(false));
  }, [category]);

  if (!category || !currentCategory) return <Navigate to="/shop/men" replace />;
        ...
        <div className="container">
            ...
            <div className="men-cat-hero__image">
              ...
              <div className="men-cat-hero__overlay">
                <div className="men-cat-hero__content">
                  ...
                  <span className="men-cat-hero__count">{allProducts.length} items available</span>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="men-cat-controls">
            <div className="men-cat-controls__left">
              ...
              <p className="men-cat-controls__count">Showing {filtered.length} of {allProducts.length} items</p>
            </div>
            <div className="men-cat-controls__right">
              <div className="men-cat-filter">
                <label htmlFor="men-price-filter">Price Range:</label>
                <select id="men-price-filter" value={priceRange} onChange={(e) => setPriceRange(e.target.value)} className="men-cat-filter__select" aria-label="Filter by price">
              ...
              <div className="men-cat-filter">
                ...
                <select id="men-sort-filter" value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="men-cat-filter__select">
          ...
          <section className="men-cat-products">
            {loading ? (
              <div className="men-cat-loading" />
            ) : filtered.length > 0 ? (
              <>
                <ProductGrid products={filtered} columns={3} />
                <div className="men-cat-actions">
                  <Link to="/shop/men" className="men-cat-back-btn">← Back to Men's Collection</Link>
                </div>
              </>
            ) : (
